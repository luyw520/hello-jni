package com.example.hellojnicallback.recyclerview.base;
...
//        AutoUtils.autoSize(holder.getConvertView());
     ...
     * @return
     */
    public CommonRecyclerViewHolder setText(int viewId, int text)
    {
        TextView tv = getView(viewId);
        tv.setText(text);
        return this;
...
//        Glide.with(mContext).load(url).centerCrop().into(view);
    ...
    public CommonRecyclerViewHolder setCircleImageUrl(int viewId, String url){
        ImageView view = getView(viewId);
//        if(hasBorder) {
//            Glide.with(mContext).load(url).centerCrop().error(R.drawable.oval_shape).transform(new CropCircleTransformation(mContext, 1.0f, mContext.getResources().getColor(R.color.white))).into(view);
//        }else{
//            Glide.with(mContext).load(url).centerCrop().error(R.drawable.oval_shape).transform(new CropCircleTransformation(mContext)).into(view);
//        }
        return this;
    }

    public CommonRecyclerViewHolder setImageUrl(int viewId, String url, int defalutRes){
        ImageView view = getView(viewId);
//        Glide.with(mContext).load(url).placeholder(defalutRes).centerCrop().into(view);
        return this;
    }
